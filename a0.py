# -*- coding: utf-8 -*-
"""
Created on Mon Jan 22 15:55:13 2018
...
consumer_key = 'MojNOji5pYnwRYM043OB3j268'
consumer_secret = 'JVYMesMQA7VUiOzR9fEyt45CpU5If2tHwR0VoAXH5FgNwJlKHI'
access_token = '954864449902596096-Iw2ycRG795v7Hn1cHwLtiNJAz4VWBzs'
access_token_secret = 'rSb9gljGHmzvUGZqPyR5qK9JxNmj9V5qjgv1VVBYnejzm'
...
def get_users(twitter, screen_names):
    ...
    for i in screen_names:
        if(i != ""):            
            userDetails = robust_request(twitter,'users/lookup',{'screen_name':i}) 
...
def get_friends(twitter, screen_name):
    ...
    """
    ###TODO
    userDetails = robust_request(twitter,'friends/ids',{'screen_name':screen_name}) 
    listoffriends= []
    for u in userDetails:
        listoffriends.append(u)
    listoffriends.sort()  
...
def friend_overlap(users):
    ...
    for i in range (0,len(users)):
        for j in range (i+1,len(users)):
            cnt = len(set(users[i]['friends']).intersection(users[j]['friends']))
            overlappedList.append((users[i]['screen_name'], users[j]['screen_name'], cnt))
    sortedList = sorted(overlappedList, key=lambda x: x[2],reverse=True)
    return sortedList
    ##pass
        ...
        if i['screen_name']=="HillaryClinton":          
            for j in users:             
                if j['screen_name']=="realDonaldTrump":                 
                    ...
                    c.update(i['friends'])
                    c.update(j['friends'])
                    friend_in_common=[k for k in i['friends'] if c[k]==2]
                    break
    #print(common_friend)    
    name = robust_request(twitter,"users/lookup",{'user_id':friend_in_common},5)
    li = [n for n in name]
...
def create_graph(users, friend_counts):
    ...
    for i in users:
        graph.add_node(i['screen_name'])
        for key, value in l:
            if(value > 1):
                graph.add_node(key)
                for x in i['friends']:
                    if(key == x):
                        graph.add_edge(key,i['screen_name'])
    return graph
    ##pass
...
def draw_network(graph, users, filename):
    ...
    list_of_users = []
    for i in users:
        list_of_users.append(i['screen_name'])
        
    pos = nx.spring_layout(graph)
    labels ={}    
    for node in graph.nodes():
        if node in list_of_users:
            labels[node] = node    
    plt.axis("off")            
    nx.draw_networkx(graph,pos,with_labels=False,node_size= 3,alpha=0.5,edge_color='r',node_color = 'black')
    nx.draw_networkx_labels(graph,pos,labels,font_size=6,font_color='b')
